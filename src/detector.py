...
# config.py 位于项目根目录，src/ 是子包，需要向上一级导入
import sys as _sys
...
class PersonDetector:
    ...
    def __init__(self):
        # 模型从 weights/ 目录加载
        model_path = str(config.WEIGHTS_DIR / config.YOLO_MODEL)
        ...
        self._annotated_frame = None   # 兼容保留
        self._last_boxes = []          # 检测框列表 [(x1,y1,x2,y2), ...]
    ...
    def _try_picamera2(self):
        ...
        if not HAS_PICAMERA2:
            logger.info("Picamera2 未安装，跳过 CSI 后端")
            return False
        try:
            picam2 = Picamera2()
            ...
            logger.warning(f"Picamera2 初始化失败: {e}")
            return False

    def _try_libcamera_pipe(self):
        """尝试 libcamera-vid 管道（CSI 摄像头，Picamera2 不可用时的备选）"""
        if sys.platform != "linux":
            return False
        import subprocess
        import shutil
        # 检查 libcamera-vid 是否可用
        if not shutil.which("libcamera-vid"):
            return False
        try:
            # 使用 libcamera-vid 输出到 stdout，格式为 MJPEG
            cmd = [
                "libcamera-vid",
                "-t", "0",           # 无限时长
                "--width", str(config.CAMERA_WIDTH),
                "--height", str(config.CAMERA_HEIGHT),
                "--framerate", str(config.CAMERA_TARGET_FPS),
                "--codec", "mjpeg",
                "--nopreview",
                "-o", "-",           # 输出到 stdout
            ]
            ...
            if self._libcamera_proc.poll() is not None:
                return False
            self.camera_backend = "libcamera-vid(CSI)"
            logger.info("摄像头后端: libcamera-vid (CSI pipe)")
            return True
        except Exception as e:
            logger.debug(f"libcamera-vid 不可用: {e}")
            return False
    ...
    def _capture_loop_libcamera(self):
        """libcamera-vid MJPEG 管道读帧循环"""
        logger.info("libcamera-vid 读帧线程已启动")
        buf = b""
        while self.running:
            try:
                chunk = self._libcamera_proc.stdout.read(4096)
                if not chunk:
                    time.sleep(0.01)
                    continue
                buf += chunk
                # 查找 JPEG 帧边界 (SOI: FF D8, EOI: FF D9)
                while True:
                ...
                logger.warning(f"libcamera 读帧异常: {e}")
                time.sleep(0.1)
    ...
    def start(self):
        ...
        elif hasattr(self, '_libcamera_proc') and self._libcamera_proc is not None:
            t_cap = threading.Thread(target=self._capture_loop_libcamera, daemon=True)
        else:
    ...
    def stop(self):
        ...
        if self.picam2:
            self.picam2.stop()
        if hasattr(self, '_libcamera_proc') and self._libcamera_proc:
            self._libcamera_proc.terminate()
            self._libcamera_proc.wait(timeout=3)
    ...
    def switch_model(self, model_path):
        ...
        # 如果是纯文件名（无路径分隔符），从 weights/ 目录解析
        is_builtin = os.path.sep not in model_path and not model_path.startswith(".")
        if is_builtin:
            resolved = str(config.WEIGHTS_DIR / model_path)
        else:
            resolved = model_path

        if not is_builtin and not os.path.exists(resolved):
            return {"ok": False, "error": f"文件不存在: {resolved}"}

        logger.info(f"开始切换模型: {resolved}")
